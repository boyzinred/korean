...
A static, GitHub Pages-hosted site of self-contained HTML lessons and quiz tools for studying Korean vocabulary and grammar (TOPIK I/II reference material, word banks, and graded reading). No build step, backend, or framework — every page is a single HTML file with inline CSS/JS that runs entirely client-side.

## Structure

- `index.html` — landing page / directory. Renders a card grid of every lesson page from a `PAGES` array (grouped into "TOPIK I", "TOPIK II", "Reading comprehension", "Helper exercises"), with collapsible sections, a jump-to-section TOC, and light/dark theme toggle (persisted in `localStorage`).
- `tutoring-topik-i-word-bank.html` — the TOPIK I word bank: 1,358 headwords over 27 study days, three exercises, and the day passages behind the question drill.
- `reading-comprehension-grade-{1,2}.html` — graded reading passages with comprehension, vocabulary, grammar, translation and question-drill exercises.
- `archive/` — pages no longer in the rotation, kept whole and still openable: the 20-lesson Bab textbook series (`tutoring-bab1.html` … `tutoring-bab20.html`) and the TOPIK I vocabulary and grammar pages it grew out of. They are off `index.html`; their relative links point up one level, so they still find `shared-vocab/` and the landing page.
